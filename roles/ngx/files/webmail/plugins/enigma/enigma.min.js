...
rcube_webmail.prototype.enigma_key_create_save=function(){var a,b,c=[],d=$("#key-pass").val();a=$("#key-pass-confirm").val();var e=$("#key-size").val();$('[name="identity[]"]:checked').each(function(){c.push({name:$(this).data("name")||"",email:$(this).data("email")})});d&&a?d!=a?this.alert_dialog(this.get_label("enigma.passwordsdiffer")):c.length?window.openpgp&&(window.msCrypto||window.crypto&&(window.crypto.getRandomValues||window.crypto.subtle))?(b=this.set_busy(!0,"enigma.keygenerating"),a={numBits:e,
userIds:c,passphrase:d},openpgp.generateKey(a).then(function(a){a={_a:"import",_keys:a.privateKeyArmored,_generated:1,_passwd:d,_keyid:a.key.primaryKey.getFingerprint()};rcmail.http_post("plugin.enigmakeys",a,b)},function(a){console.error(a);rcmail.set_busy(!1,null,b);rcmail.display_message(rcmail.get_label("enigma.keygenerateerror"),"error")})):rcmail.display_message(rcmail.get_label("enigma.keygennosupport"),"error"):this.alert_dialog(this.get_label("enigma.noidentselected")):this.alert_dialog(this.get_label("enigma.formerror"))};
rcube_webmail.prototype.enigma_key_create_success=function(){parent.rcmail.enigma_list(1)};rcube_webmail.prototype.enigma_delete=function(){var a=this.keys_list.get_selection();a.length&&this.confirm_dialog(this.get_label("enigma.keyremoveconfirm"),"delete",function(b,c){var d=c.display_message(c.get_label("enigma.keyremoving"),"loading");c.http_post("plugin.enigmakeys",{_a:"delete",_keys:a},d)})};
...
return this.show_popup_dialog(a.text,a.title,b,{width:500,dialogClass:"error"})};rcube_webmail.prototype.enigma_find_publickey=function(a){this.mailvelope_search_pubkeys([a],function(a){},function(a){var c=rcmail.set_busy(!0,"enigma.importwait");a={_a:"import",_keys:a};"plugin.enigmakeys"==rcmail.env.action&&(a._refresh=1);rcmail.http_post("plugin.enigmakeys",a,c)})};
